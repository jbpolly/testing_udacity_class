...
class TasksDaoTest {
    ...
    //Database test usually instrumented
    //Unit test DAO
    ...
    fun updateTaskAndGetById() = runBlockingTest{
        ...
        val task = Task("title", "description", id = "test")
        database.taskDao().insertTask(task)

        // 2. Update the task by creating a new task with the same ID but different attributes.
        val taskUpdated = Task("title_up", "description_up", id = "test")
        database.taskDao().insertTask(taskUpdated)

        // 3. Check that when you get the task by its ID, it has the updated values.
        val loaded = database.taskDao().getTaskById(taskUpdated.id)
        assertThat<Task>(loaded as Task, notNullValue())
